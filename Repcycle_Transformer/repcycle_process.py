...
def process_repcycles(waveform_t, vec_size):
...
# GPU-accelerated variant: keep original algorithm but run numeric work on a torch device
def process_repcycles_FAST_FFT_NOISY_GBS_gpu(waveform_t, vec_size, device=None):
  """GPU-friendly version of process_repcycles_FAST_FFT_NOISY_GBS.
  - waveform_t is expected to be a torch tensor similar to the original function input.
  - vec_size is the output vector length per repcycle.
  - device: torch.device or string (e.g. 'cuda') — if None uses CUDA when available.

  This implementation keeps the original control flow but uses torch tensors and
  torch.fft to accelerate heavy array work on the chosen device. It still iterates
  over segments in Python as the algorithm is inherently sequential per-segment.
  """
  if device is None:
  ...
  # bring waveform to device and into 1D tensor shape
  waveform = waveform_t.t().squeeze(1).to(device)

  repcycles_t = torch.zeros(N_SEGMENTS, vec_size)

  def _torch_fft_max(seg_wav, lower_bound, upper_bound):
    # seg_wav: 1D torch tensor on device
    L = seg_wav.size(0)
    if L < FFT_SIZE:
      pad = FFT_SIZE - L
      seg = torch.cat([seg_wav, torch.zeros(pad, device=seg_wav.device)])
    else:
      seg = seg_wav[:FFT_SIZE]

    fft = torch.fft.fft(seg, n=FFT_SIZE)
    fft_mag = torch.abs(fft)[:FFT_SIZE // 2]
    bounded = fft_mag[lower_bound:upper_bound]
    if bounded.numel() == 0:
      return lower_bound, fft_mag
    max_idx = int(torch.argmax(bounded).item())
    return lower_bound + max_idx, fft_mag

  def _vectorize_f_torch(seg_wav, n):
    # Linear interpolation of seg_wav to n points using torch arithmetic
    # Returns a tensor on the same device and dtype as seg_wav (no host transfers).
    L = seg_wav.size(0)
    device_loc = seg_wav.device
    dtype_loc = seg_wav.dtype
    if L == 0:
      return torch.zeros(n, device=device_loc, dtype=dtype_loc)
    if L == 1:
      return seg_wav.repeat(n).to(device_loc)

    sample_points = torch.linspace(0.0, float(L - 1), steps=n, device=device_loc)
    idx0 = torch.floor(sample_points).long()
    idx1 = torch.clamp(idx0 + 1, max=L - 1)
    alpha = (sample_points - idx0.float())

    v0 = seg_wav[idx0]
    v1 = seg_wav[idx1]
    # alpha shape is (n,), v0/v1 are (n,) — broadcast works directly
    values = (1.0 - alpha) * v0 + alpha * v1
    return values.to(dtype=dtype_loc, device=device_loc)

  for segment_num in range(N_SEGMENTS):
    start_sample = segment_num * SEGMENT_LENGTH
    segment_wav = waveform[start_sample:start_sample + SEGMENT_LENGTH]
    ...
    # Note: we still use Python loops for the search logic; the arithmetic inside
    # the loop is performed on device (via tensor indexing and torch.fft).
    while 1 < index - offset_index and index + offset_index < segment_wav.size(0) and not exit_flag:
      for sign in (1, -1):
        i1 = int(index - 1 + sign * offset_index)
        i2 = int(index + sign * offset_index)
        if i1 < 0 or i2 < 0 or i1 >= segment_wav.size(0) or i2 >= segment_wav.size(0):
          continue

        # pull scalar values to Python for sign checks (cheap)
        last = float(segment_wav[i1].item())
        value = float(segment_wav[i2].item())
        if (last < 0.0) and (value > 0.0):
          inter_x = (i1) + (-last / (value - last))
          if repcycle[0] is None:
            repcycle = (start_sample + inter_x, None)
            # compute f0 using torch FFT on the device
            try:
              max_index, _ = _torch_fft_max(segment_wav, LOW_F0_INDEX, HIGH_F0_INDEX)
            except Exception:
              max_index = LOW_F0_INDEX

            # Fundamental Frequency: Sample Rate / fftsize * fft_bin_index
            f0 = AUDIO_SIZE / FFT_SIZE * max_index if max_index != 0 else 1.0
            f0_length = 1.0 / f0 * AUDIO_SIZE
            index += (offset_index + int(f0_length))
            offset_index = 0
            break
    ...
    repc_start = math.floor(repcycle[0])
    repc_end = math.floor(repcycle[1])
    # Slice on device and vectorize with torch-based interpolation
    seg_slice = waveform[repc_start:repc_end]
    repc_wav = _vectorize_f_torch(seg_slice, vec_size)

    # repc_wav is already on-device; copy directly into the preallocated device tensor
    repcycles_t[segment_num] = repc_wav
