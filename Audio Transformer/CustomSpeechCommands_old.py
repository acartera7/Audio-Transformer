...
class CustomSpeechCommandsDataset_R(Dataset):
  def __init__(self, base_dir: str, subset: str = None, n_segments=32, shuffle: bool = False, vec_size=40, divisor: int = 1, control: bool = False):
    ...
    self.repc_base_dir = Path(base_dir + f"_repcycles{self.n_segments}")  

    if not self.repc_base_dir.exists() or not self.repc_base_dir.is_dir() :
      raise FileNotFoundError(f"The directory '{str(self.repc_base_dir)}' does not exist. Please make sure audio files are converterd using Dataset2Cycles.py")
    print(f"Using repcycles from: {str(self.repc_base_dir)}")
    self.subset = subset
...
if __name__ == "__main__":
  ...
  dataset = CustomSpeechCommandsDataset_R(base_dir=base_dir, n_segments=int(sys.argv[1]), subset="training", shuffle=True, vec_size=int(sys.argv[3]), divisor=32)
