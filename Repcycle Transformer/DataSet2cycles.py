...
#usage python DataSet2cycles.py <path_to_dataset> <params_file>
if __name__ == "__main__":

  # Instantiate the parser TODO add description
  parser = argparse.ArgumentParser(prog='Dataset2Cycles', description='TODO Optional app description')
  parser.add_argument('path_to_dataset', type=str, help='Path to the dataset folder')
  parser.add_argument('params_file', type=str, help='Path to the parameters file')
  parser.add_argument('-v', '--verbose', action='store_true', help='Enable verbose mode')
  parser.add_argument('-q', '--quick', action='store_true', help='Enable quick mode where the repcycle is found by looking for the cycle closest to the middle of the segment')
  ...
  if not os.path.exists(path_to_dataset):
    print(f"Error: {path_to_dataset} does not exist.")
    sys.exit(1)
  ...
  with open(params_file, 'r') as f:
    ...
    F0_FREQ_RANGE = config['F0_FREQ_RANGE']
  ...
  # Loop through the entries in the main folder
  for root in os.listdir(path_to_dataset):

    if args.quick:
      path_to_OUT_dataset = path_to_dataset + f"_repcycles{N_SEGMENTS}_Quick"
    else:
      path_to_OUT_dataset = path_to_dataset + f"_repcycles{N_SEGMENTS}"
    os.makedirs(path_to_OUT_dataset, exist_ok=True)

    root_path = os.path.join(path_to_dataset, root)

    # Check if the entry is a directory
    if os.path.isdir(root_path):
      if args.verbose:
        print(f"===== Entering directory: {root_path} =====")
      
      # Loop through the files in the subdirectory
      for dir, subdirs, files in os.walk(root_path):
        
        out_dir = os.path.join(path_to_OUT_dataset,dir.split('\\')[-1])
        ...
        for file in files:
          ...
          waveform_t, _ = torchaudio.load(file_path)
          repcycles = process_repcycles(waveform_t, args.quick)

          filename = file.split('.')[0]
          with open(os.path.join(out_dir,f'{filename}.txt'), 'w') as file:
  ...
  print(f"===== Process Sucessfully Completed! =====")
